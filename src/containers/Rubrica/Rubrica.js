...
import TopBar from "../../components/UI/TitleBar/TitleBar";
...
class Rubrica extends Component {
  state = {
    ...
    renglones: [
      { id: 1, criterio: "Excelente", num_renglon: 1, id_instrumento: 1 },
    ],
    celdas: [
      {
        id: 11,
  ...
  render() {
    const columnas = this.state.columnas.map((columna) => (
      <Grid item xs={12} sm key={columna.id}>
        <ColumnaInstrumento
          celdas={this.state.celdas.filter(
            (celda) => celda.id_columna === columna.id
          )}
